...
> `position`이 `INTEGER`이므로 "간격이 1 미만"은 실질적으로 **값이 같아지는 경우**를 뜻한다.
> 클라이언트가 `(5 + 6) / 2 = 5.5`를 내림하면 `5`가 되어 앞 카드와 충돌한다.
> 서버는 이동 후 칼럼 내에 중복 `position`이 생기면 0부터 1024 간격으로 다시 매긴다.
...
**200 OK** — 갱신된 보드 전체 ([2.2 BoardData](#22-boarddata-객체))

`GET /api/tickets`와 동일한 형태를 반환한다.

칼럼 간 이동은 두 칼럼이 함께 바뀌고 재정렬은 칼럼 전체가 바뀌므로,
변경 범위를 열거하는 것보다 보드 전체를 돌려주는 편이 정확하다.
클라이언트는 응답을 보드 상태에 그대로 대입하면 되고 병합 로직이 필요 없다.
...
`DONE` 배열은 보드 조회와 같은 규칙으로 24시간 이내 완료분만 포함한다.
...
### 13.2 `reorder` 응답 범위 — 해소됨

FR-007의 성공 응답이 "업데이트된 티켓 목록"으로만 적혀 있어 범위가 모호했다.
**`BoardData` 전체**를 반환하는 것으로 확정했다(9.3).

칼럼 간 이동은 두 칼럼이 함께 바뀌고 재정렬은 칼럼 전체가 바뀌므로, 변경 범위를
정확히 열거하기 어렵다. 보드 전체를 돌려주면 클라이언트가 응답을 상태에 그대로
대입할 수 있어 병합 로직이 사라진다.
