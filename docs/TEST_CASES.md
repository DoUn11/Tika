...
| 001-N7 | 드래그 중 | 카드 미리보기가 함께 보인다 (`DragOverlay`) |
| 001-E1 | 빈 칼럼으로 이동 | `position = 0`으로 요청된다 |

> **001-N7이 검증하는 것과 못 하는 것.** COMPONENT_SPEC 4.6은 드래그 중
> 원래 자리에 반투명 placeholder를 두고 커서에는 `DragOverlay`를 띄우라고 한다.
> jsdom은 레이아웃이 없어 "커서를 따라간다"를 확인할 수 없으므로, **미리보기가
> 존재하는지**까지만 본다. 이 케이스는 뒤늦게 추가했다 — `DragOverlay` 없이도
> 통합 테스트가 모두 통과했고, 브라우저에서 만져 보고서야 카드가 커서를 따라오지
> 않고 드롭 순간 순간이동한다는 것이 드러났다.
