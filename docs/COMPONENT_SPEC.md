...
24시간이 지난 완료 티켓이 사라진 것을 삭제로 오해하지 않게 하기 위해서다 (3.6).

### 3.5 접근성 (NFR-003)

칼럼은 `<section>`으로 감싸고 헤더 제목으로 이름을 붙인다. 이름이 있는 `section`은
`region` 랜드마크가 되므로, 스크린 리더 사용자가 칼럼 단위로 건너뛸 수 있다.

```html
<section aria-labelledby="column-BACKLOG">
  <h2 id="column-BACKLOG">Backlog</h2>
  <span>3</span>          <!-- 카드 수는 제목 밖에 둔다 -->
  ...
</section>
```

카드 수를 `h2` 안에 넣지 않는 이유는, 랜드마크 이름이 `"Backlog 3"`처럼
개수까지 포함해 읽히면 칼럼을 식별하기 어려워지기 때문이다.
...
| 로딩 표현 | 스켈레톤 또는 스피너를 모달 본문에 표시한다. 형태와 무관하게 `role="status"`로 알린다 (NFR-003) |
| 실패 처리 | 조회에 실패하면 `ApiError`의 `message`를 본문에 표시한다 (404이면 "티켓을 찾을 수 없습니다") |

**조회 실패 시 모달을 닫지 않는다.** 닫아 버리면 사용자가 왜 열리지 않았는지 알 수 없다.
본문에 문구를 띄우고 "닫기"만 남겨 사용자가 직접 닫게 한다.
문구는 서버 응답을 그대로 쓴다 (6.4).
...
|------|------|------|------|
| 편집 가능 | `title` | 제목 | O |
| 편집 가능 | `description` | 설명 | O |
| 편집 가능 | `priority` | 우선순위 | O |
| 편집 가능 | `plannedStartDate` | 시작예정일 | O |
| 편집 가능 | `dueDate` | 종료예정일 | O |
| 읽기 전용 | `startedAt` | 시작일 | **X** — 칼럼 이동으로 자동 기록 |
| 읽기 전용 | `completedAt` | 종료일 | **X** — 칼럼 이동으로 자동 기록 |
| 읽기 전용 | `status`, `createdAt`, `updatedAt` | - | X |

`startedAt`·`completedAt`은 `YYYY-MM-DD HH:mm`으로 표시하고, 값이 없으면 `-`를 보인다.
라벨 "종료일"(`completedAt`)과 "종료예정일"(`dueDate`)은 서로 다른 필드다.

**저장 시 바뀐 필드만 보낸다**

`PATCH`는 전송된 필드만 갱신하므로(FR-004) 손대지 않은 필드는 보내지 않는다.

단순히 규약을 따르는 문제가 아니다. `updateTicketSchema`의 `dueDate`는
**오늘 이후**만 허용하므로, 값을 그대로 되돌려 보내면 **오버듀 티켓은 제목조차
고칠 수 없게 된다.** 지난 종료예정일이 검증에 걸려 400이 나기 때문이다.
